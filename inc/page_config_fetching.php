...
if(isset($_POST) && isset($_SERVER['HTTP_X_REQUESTED_WITH']) && strtolower($_SERVER['HTTP_X_REQUESTED_WITH']) == 'xmlhttprequest'){
    ...
    $get_total_rows = $mess->GetMassMessagesCount(); //hold total records in variable
    //break records into pages
    $total_pages = ceil($get_total_rows/$item_per_page);
    //get starting position to fetch the records
    $page_position = (($page_number-1) * $item_per_page);
    //Limit our results within a specified range.
    $results = $db->query("SELECT * FROM mass_mess ORDER BY id DESC LIMIT $page_position,$item_per_page");
    ...
    <tr>
        <th>#</th>
        <th class="hidden-phone">پیام</th>
        <th class="">اتمام یافته</th>
        <th class="">تعداد گروه ها</th>
        <th class="">تعداد اعضا</th>
        <th class="">بیشتر</th>
    </tr>
    </thead>
    <tbody>
...
?>
            <tr class="odd gradeX">
                ...
                <td class="hidden-phone"><?php echo substr($row['message'],0,20);?></td>
                <td class=""><span class="label label-success"><?php echo $f;?></span></td>
                <td class=""><?php echo $row['groups_count'];?></td>
                <td class=""><?php echo $row['users_count'];?></td>
                <td class=""><a href="messages.php?mass_id=<?php echo $row['id'];?>" target="_blank"><i class="icon-location-arrow"></i></a></td>
            </tr>
    <?php
    }
    ?>
